...
if (isset($_POST['comprar'])) {
    ...
    if ($datos !== false) {
        if (isset($_COOKIE['familia'])){ 
            $familia_arr = $_COOKIE['familia'];
        } else { $familia_arr = array();}
        if (!in_array($datos->familia, $familia_arr)){
            array_push($familia_arr, $datos->familia);
            $id_familia = array_key_last($familia_arr);
            setcookie("familia[$id_familia]", $datos->familia, time() + 2592000);
        }
